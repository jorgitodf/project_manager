  ...
  <v-card color="blue-grey lighten-4">
    ...
    <v-container>
      <v-form ref="form" v-model="valid" xs12>
        <v-text-field id="project-title" v-model="data.title" :rules="validation.title" label="Título" required>
        </v-text-field>
        <div v-show="data.title">
          <v-text-field v-model="data.description" label="Descrição" outline></v-text-field>

          <v-menu ref="menu" v-model="menu" :close-on-content-click="false" :return-value.sync="due_date">
            <v-text-field slot="activator" v-model="due_date" label="Data da Entrega" readyonly></v-text-field>
          ...
          <v-menu ref="menuTime" v-model="menu2" :close-on-content-click="false" :return-value.sync="due_date_time">
